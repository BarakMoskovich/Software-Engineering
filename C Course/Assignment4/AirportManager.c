...
int		saveManagerToFile(const AirportManager* pManager, const char* fileName)
{
	...
	while(pNode)
	{
		...
		CHECK_0_MSG_COLSE_FILE(fp, "Error write airport\n", saveAirportToFile(pNode->key, fp))
...
int		loadManagerFromFile(AirportManager* pManager, const char* fileName)
{
	...
	for (int i = 0; i < pManager->count; i++)
	{
		...
		CHECK_0_MSG_COLSE_FILE(fp, "Error loading airport from file\n", loadAirportFromFile(pPort, fp))
